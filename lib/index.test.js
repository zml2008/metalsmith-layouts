...
describe('metalsmith-layouts', () => {
  ...
  it('should use the default extension when one is not provided', done => {
    const base = path.join(process.cwd(), 'test', 'fixtures', 'default-extension');
    const actual = path.join(base, 'build');
    const expected = path.join(base, 'expected');
    const metalsmith = new Metalsmith(base);
    ...
    return metalsmith.use(plugin({ defaultExtension: 'hbs' })).build(err => {
      if (err) {
        return done(err);
      }
      expect(() => equal(actual, expected)).not.toThrow();
