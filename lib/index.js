 ...
 * 3. Layouts without an extension, by using the default extension
 * 3. Default layout in the settings
 */

function getLayout({ file, settings }) {
  if (file.layout === false) {
    return false;
  }

  if (file.layout) {
    if (!file.layout.includes('.') && settings.defaultExtension !== null) {
      return `${file.layout}.${settings.defaultExtension}`;
    }
    return file.layout;
  }
...
function validate({ filename, files, settings }) {
  ...
  if (!transformer) {
    debug(
      `validation failed, no jstransformer found for layout for ${filename}, ${layout} with extension ${extension}`
    );
  }
